...
function simulate(p: Params) {
  ...
  for (let m = 1; m <= months; m++) {
    ...
    // 住宅ローン控除：前年末残高 × 0.7%、上限35万円（ZEH）、13年間
    const mortgageDeductionAmt = year <= 13
      ? Math.min(loanBalance * 0.007, 350000)
      : 0
            ...
            <div style={{ paddingTop: 16 }}>
              ...
              <ResponsiveContainer width="100%" height={230}>
                <AreaChart data={yearlyData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                  ...
                  {/* ローン残高を先に描くことで差分エリアを重ねられる */}
                  <Area
                    type="monotone" dataKey="loanBalance"
                    stroke={RED} strokeWidth={2} strokeDasharray="5 3"
                    fill="none"
                    name="ローン残高" dot={false}
                  />
                  {/* 物件価値 — ローン残高との差分が純資産として塗られる */}
                  <Area
                    type="monotone" dataKey="propertyValue"
            ...
            <>
              ...
              <div style={{ background: '#1c1c1e', borderRadius: 12, padding: '12px 16px', marginBottom: 28 }}>
                ...
                <p style={{ fontSize: 12, color: '#48484a', lineHeight: 1.7 }}>
                  年末ローン残高 × 0.7%（上限35万円）で自動計算。<br />
                  ZEH住宅・13年間の控除を適用しています。
                </p>
              </div>
